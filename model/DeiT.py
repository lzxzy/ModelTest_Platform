...
class PatchEmbedding(nn.Module):
    def __init__(self, in_channels:int = 3, patch_size=16, emb_size=768, img_size=224):
        ...
        self.dstl_token = nn.Parameter(torch.randn(1,1, emb_size))
        self.position = nn.Parameter(torch.randn((img_size//patch_size)**2+2, emb_size)) # Absolute position token: length = patch_num + cls_token + distillation token
    def forward(self, x):
        b, _, _, _ = x.shape
        x = self.projections(x)
        cls_tokens = repeat(self.cls_token, '() n e -> b n e', b = b)
        dstl_tokens = repeat(self.dstl_token, '() n e -> b n e', b = b)
...
class ClassificationHead(nn.Module):
    ...
    def forward(self, x):
        cls_token = x[:,0,:]
        cls_result = self.classify(cls_token)
        return cls_result
